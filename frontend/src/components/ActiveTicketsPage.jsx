...
function ActiveTicketsPage({ onSelectTicket }) {
  ...
  useEffect(() => {
    ...
    async function loadActiveTickets() {
      ...
      try {
        const data = await getTickets({ status_not: 'Closed', page, limit: PAGE_LIMIT })
        if (active) {
  ...
  if (loading) {
    return <p>Loading active tickets...</p>
  }
  ...
  if (tickets.length === 0) {
    return <p>No active tickets.</p>
  }

  return (
    <div className="active-tickets-page">
